...
public class AddActivity2 extends AppCompatActivity {
    ...
    private void insertData() {
        ...
        FirebaseDatabase.getInstance().getReference().child("movil_tope").push().setValue(map).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Toast.makeText(AddActivity2.this, "Datos Creados correctamente", Toast.LENGTH_SHORT).show();
            }
        })
                .addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(Exception e) {
                        Toast.makeText(AddActivity2.this, "Ocurrio un error al insertar los datos", Toast.LENGTH_SHORT).show();
                    }
                });
